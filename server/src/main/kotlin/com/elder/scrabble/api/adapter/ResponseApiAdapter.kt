package com.elder.scrabble.api.adapter;

import com.elder.scrabble.core.logOnError
import com.elder.scrabble.core.logOnNext
import com.elder.scrabble.core.logOnSubscribe
import com.elder.scrabble.core.logOnSuccess
...
class ResponseApiAdapter(private val validator: Validator) {
    ...
    private fun handleResponse(response: Response): Mono<ServerResponse> = when(response) {
        is Response.SuccessResponse -> ServerResponse.ok().body(response.payload.toMono())
    ...
    sealed class Response {
        ...
        data class SuccessResponse(val payload: Any = emptyMap<String, String>()) : Response()

        data class ClientFailure(val payload: ErrorResponse) : Response()
        data class ServerFailure(val payload: ErrorResponse) : Response()

        companion object {
            fun successResponse(payload: Any): Response = SuccessResponse(payload = payload)
        }
